            ...
            if (!$([System.IO.File]::Exists("$downloadPath\ApplicationIDBackup.txt"))) {
                if (($authenticationType.ToString() -like "AzureAd") -and ($deploymentMode -ne "Offline")) {
                    ...
                    $appID = . .\Create-AADIdentityApp.ps1 -DirectoryTenantName "$azureDirectoryTenantName" -AdminArmEndpoint "adminmanagement.$regionName.$externalDomainSuffix" -TenantArmEndpoint "management.$regionName.$externalDomainSuffix" `
                        -CertificateFilePath "$AppServicePath\sso.appservice.$regionName.$externalDomainSuffix.pfx" -CertificatePassword $secureVMpwd -AzureStackAdminCredential $asdkCreds -Verbose
                ...
                elseif ($authenticationType.ToString() -like "ADFS") {
                    ...
                    $appID = .\Create-ADFSIdentityApp.ps1 -AdminArmEndpoint "adminmanagement.$regionName.$externalDomainSuffix" -PrivilegedEndpoint $ERCSip `
                        -CertificateFilePath "$AppServicePath\sso.appservice.$regionName.$externalDomainSuffix.pfx" -CertificatePassword $secureVMpwd -CloudAdminCredential $asdkCreds -Verbose
                    Write-Host "Saving the application ID to a backup file."
                    $appIdPath = "$downloadPath\ApplicationIDBackup.txt"
